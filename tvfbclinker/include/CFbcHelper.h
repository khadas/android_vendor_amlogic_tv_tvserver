...
#ifndef _C_FBC_HELPER_H_
#define _C_FBC_HELPER_H_
...
/**
 * @Function sendCommandToFBC(unsigned char *data, int count, int flag)
 * @Description send special command from tv to fbc.
 * @Param data COMM_DEV_TYPE_E + fbc_command_t + values
 * @Param count count of values from {@Param data}
 * @Param flag 1 get value from fbc, 0 set value to fbc
 * @Return -1 failed, 0 successful
 */
extern int sendCommandToFBC(unsigned char *data, int count, int flag);

/**
* @Function startFBCUpgrade(char *file_name, int mode, int upgrade_blk_size)
* @Description upgrade fbc from tv.
* @Param file_name name of fbc bin.
* @Param blk_size upgrade block size (min is 4KB)
* @Param mode one type of enum fbc_upgrade_mode_t, default is all(value = 4).
* @Return -1 failed, 0 successful
*/
extern int startFBCUpgrade(char *file_name, int blk_size, int mode = 4);

extern int setUpgradeFbcObserver(IUpgradeFBCObserver *pOb);
